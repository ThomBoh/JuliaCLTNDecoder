...
function SurfCirc(dz,dx,nr,PEZ,PEX,Synz,Synx,zsch,xsch,bsch,layout,ql,zl,xl,p,al2,tmeas,k2,nth,pmz,pmx,acc,bd)
    ...
    return TOut,elapsed #return the MPS, whose physical indices are as described at top of function
end
...
function SurfMC(dz,dx,nr,p,al2,tmeas,k2,nth,acc,bd,err,nt; sim_id::Int=-1)
    ...
    while n<nt #breakflag==0
        ...
        #println(PEZ)
        #elapsed = @elapsed begin
        MPS,elapsed=SurfCirc(dz,dx,nr,PEZ,PEX,Synz,Synx,zsch,xsch,bsch,layout,ql,zl,xl,p,al2,tmeas,k2,nth,pmz,pmx,acc,bd)
        #end
	println(elapsed)
	if n>0
